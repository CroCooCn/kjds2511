...
public class ShippingTemplatesRequest implements Serializable {
    ...
    @Min(value = 0, message = "计费方式选择区间 1(按件数), 2(按重量)，3(按体积)")
    ...
    @Min(value = 0, message = "未知的包邮类型")
    @Max(value = 2, message = "未知的包邮类型")
    private Integer appoint;

    @ApiModelProperty(value = "排序", example = "0")
    @NotNull(message = "排序数字必须填写")
    @Min(value = 0, message = "排序范围为0~9999")
    @Max(value = 9999, message = "排序范围为0~9999")
